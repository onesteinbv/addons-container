...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _get_consolidation_groups_data(
        self, accounts_data, total_amount, foreign_currency
    ):
    ...
    def _compute_consolidation_accounts_data(
        self, consolidation_account_ids, data, accounts_data
    ):
        ...
        consolidation_accounts = self.env["consolidation.account"].search(
            accounts_domain
        )
        ...
        for key in accounts_data:
            ...
            consolidation_account = account.consolidation_account_ids.filtered(
            ...
            if (
                ...
                if not consolidation_account.id in new_accounts_data:
                    ...
                    if show_hierarchy:
                        new_accounts_data[consolidation_account.id].update(
                            {
                                "complete_code": consolidation_account.group_id
                                and (
                                    consolidation_account.group_id.complete_code
                                    + " / "
                                    + consolidation_account.code
                                )
                                or ""
                            }
                        )
                ...
                if not show_partner_details:
                    new_accounts_data[consolidation_account.id].update(
                        {
                            "initial_balance": new_accounts_data[
                                consolidation_account.id
                            ]["initial_balance"]
                            + item["initial_balance"],
                            "credit": new_accounts_data[consolidation_account.id][
                                "credit"
                            ]
                            + item["credit"],
                            "debit": new_accounts_data[consolidation_account.id][
                                "debit"
                            ]
                            + item["debit"],
                            "balance": new_accounts_data[consolidation_account.id][
                                "balance"
                            ]
                            + item["balance"],
                            "ending_balance": new_accounts_data[
                                consolidation_account.id
                            ]["ending_balance"]
                            + item["ending_balance"],
                        }
                    ...
                    if foreign_currency:
                        new_accounts_data[consolidation_account.id].update(
                            {
                                "ending_currency_balance": new_accounts_data[
                                    consolidation_account.id
                                ]["ending_currency_balance"]
                                + item["ending_currency_balance"],
                                "initial_currency_balance": new_accounts_data[
                                    consolidation_account.id
                                ]["initial_currency_balance"]
                                + item["initial_currency_balance"],
                            }
                        )

        return new_accounts_data

    def _compute_consolidation_total_amount(
        self, consolidation_account_ids, data, accounts_data, total_amount
    ):
        chart_id = data["chart_id"]
        show_partner_details = data["show_partner_details"]
        foreign_currency = data["foreign_currency"]
        ...
        for key in total_amount:
            ...
            if (
                ...
                new_total_amount[consolidation_account.id].update(
                    {
                        "initial_balance": new_total_amount[consolidation_account.id][
                            "initial_balance"
                        ]
                        + item["initial_balance"],
                        "credit": new_total_amount[consolidation_account.id]["credit"]
                        + item["credit"],
                        "debit": new_total_amount[consolidation_account.id]["debit"]
                        + item["debit"],
                        "balance": new_total_amount[consolidation_account.id]["balance"]
                        + item["balance"],
                        "ending_balance": new_total_amount[consolidation_account.id][
                            "ending_balance"
                        ]
                        + item["ending_balance"],
                    }
                ...
                if foreign_currency:
                    new_total_amount[consolidation_account.id].update(
                        {
                            "initial_currency_balance": new_total_amount[
                                consolidation_account.id
                            ]["initial_currency_balance"]
                            + item["initial_currency_balance"],
                            "ending_currency_balance": new_total_amount[
                                consolidation_account.id
                            ]["ending_currency_balance"]
                            + item["ending_currency_balance"],
                        }
                    ...
                    for key in item:
                        if isinstance(item[key], dict):
                            ...
                            new_total_amount[consolidation_account.id][key].update(
                                {
                                    "credit": new_total_amount[
                                        consolidation_account.id
                                    ][key]["credit"]
                                    + partner_item["credit"],
                                    "debit": new_total_amount[consolidation_account.id][
                                        key
                                    ]["debit"]
                                    + partner_item["debit"],
                                    ...
                                    + partner_item["balance"],
                                    "initial_balance": new_total_amount[
                                        consolidation_account.id
                                    ][key]["initial_balance"]
                                    + partner_item["initial_balance"],
                                    "ending_balance": new_total_amount[
                                        consolidation_account.id
                                    ][key]["ending_balance"]
                                    + partner_item["ending_balance"],
                                }
                            )
                            if foreign_currency:
                                new_total_amount[consolidation_account.id][key].update(
                                    {
                                        ...
                                        + partner_item["initial_currency_balance"],
                                        "ending_currency_balance": new_total_amount[
                                            consolidation_account.id
                                        ][key]["ending_currency_balance"]
                                        + partner_item["ending_currency_balance"],
                                    }
                                )

        if show_partner_details and foreign_currency:
            for account_id in accounts_data.keys():
                new_total_amount[account_id]["currency_id"] = accounts_data[account_id][
                    "currency_id"
                ]
                new_total_amount[account_id]["currency_name"] = accounts_data[
                    account_id
                ]["currency_name"]
    ...
    def _compute_consolidation_trial_balance(self, data, accounts_data, total_amount):
        ...
        if not show_partner_details:
            ...
            if show_hierarchy:
                ...
                new_trial_balance = sorted(
                    new_trial_balance, key=lambda k: k["complete_code"]
                )
    ...
    def _get_report_values(self, docids, data):
        ...
        if data["schema_type"] == "consolidation":
            res["accounts_data"] = self._compute_consolidation_accounts_data(
                data["consolidation_account_ids"], data, res["accounts_data"]
            )
            res["total_amount"] = self._compute_consolidation_total_amount(
                data["consolidation_account_ids"],
                data,
                res["accounts_data"],
                res["total_amount"],
            )
            res["trial_balance"] = self._compute_consolidation_trial_balance(
                data, res["accounts_data"], res["total_amount"]
            )
        return res
