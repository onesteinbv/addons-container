...
class ConsolidationGroup(models.Model):
    ...
    parent_id = fields.Many2one('consolidation.group', index=True, ondelete='cascade', readonly=True)
    ...
    def _constraint_prefix_overlap(self):
        ...
        if res:
            raise ValidationError(_('Consolidation Groups with the same granularity can\'t overlap'))
    ...
    def unlink(self):
        for record in self:
            account_ids = self.env['consolidation.account'].search([('group_id', '=', record.id)])
            account_ids.write({'group_id': record.parent_id.id})

            children_ids = self.env['consolidation.group'].search([('parent_id', '=', record.id)])
    ...
    def _adapt_accounts_for_consolidation_groups(self, account_ids=None):
        ...
        self.env['consolidation.account'].flush(self.env['consolidation.account']._fields)
    ...
    def _adapt_parent_consolidation_group(self):
       ...
       SELECT DISTINCT FIRST_VALUE(parent.id) OVER (PARTITION BY child.id ORDER BY child.id, char_length(parent.code_prefix_start) DESC) AS parent_id,
        ...
        self.env['consolidation.group'].search([('chart_id', 'in', self.chart_id.ids)])._parent_store_update()
