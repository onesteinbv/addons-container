...
if [[ -n "$KEYCLOAK_URL" ]]; then
  python /odoo/scripts/setup_keycloak.py -c $ODOO_RC -d $DB_NAME --log-level=error --url "$KEYCLOAK_URL" --realm "$KEYCLOAK_REALM" --client-id "$KEYCLOAK_CLIENT_ID" --client-secret "$KEYCLOAK_CLIENT_SECRET"
fi
